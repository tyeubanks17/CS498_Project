...
class StatsTracker:
    ...
    def get_all_time_accuracy(self, set_name):
        ...
        if not stats:
            return 0
        total_correct = sum(t["correct"] for t in stats.values())
        total = sum(t["correct"] + t["incorrect"] for t in stats.values())
    ...
    def get_all_sets_summary(self):
        ...
        for set_name in self.all_time_stats.keys():
            stats = self.get_stats(set_name)
            total_correct = sum(t["correct"] for t in stats.values())
            total_answers = sum(t["correct"] + t["incorrect"] for t in stats.values())
    ...
    def get_total_correct_across_sets(self):
        ...
        for set_name in self.all_time_stats.keys():
            stats = self.get_stats(set_name)
            total += sum(t["correct"] for t in stats.values())
        return total
